...
/// @title ItsARace
/// @notice This contract is an older instance from the Surge lending pool protocol. 
contract ItsARace {
    ...
    constructor(
        ...
        require(_collateralToken != _loanToken, "ItsARace: collateral and loan tokens are the same");
        require(_collateralRatioFallDuration > 0, "ItsARace: _collateralRatioFallDuration too low");
        require(_collateralRatioRecoveryDuration > 0, "ItsARace: _collateralRatioRecoveryDuration too low");
        require(_maxCollateralRatioMantissa > 0, "ItsARace: _maxCollateralRatioMantissa too low");
        require(_surgeMantissa < 1e18, "ItsARace: _surgeMantissa too high");
        require(_minRateMantissa <= _surgeRateMantissa, "ItsARace: _minRateMantissa too high");
        require(_surgeRateMantissa <= _maxRateMantissa, "ItsARace: _surgeRateMantissa too high");
        require(_maxRateMantissa <= RATE_CEILING, "ItsARace: _maxRateMantissa too high");
        symbol = _symbol;
        name = _name;
    ...
    function safeTransfer(IERC20 token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'ItsARace: TRANSFER_FAILED');
    }
    ...
    function safeTransferFrom(IERC20 token, address from, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'ItsARace: TRANSFER_FROM_FAILED');
    }
    ...
    function transfer(address to, uint amount) external returns (bool) {
        require(to != address(0), "ItsARace: to cannot be address 0");
    ...
    function deposit(uint amount) external {
        ...
        (address _feeRecipient, uint _feeMantissa) = (address(0x111), 0);
        (  
            uint _currentTotalSupply,
            uint _accruedFeeShares,
            uint _currentCollateralRatioMantissa,
        ...
        require(_shares > 0, "ItsARace: 0 shares");
    ...
    function removeCollateral(uint amount) external {
        ...
        if(userDebt > 0) {
            ...
            require(userCollateralRatioMantissa <= _currentCollateralRatioMantissa, "ItsARace: user collateral ratio too high");
        }
    ...
    function borrow(uint amount) external {
        ...
        require(_newUtil <= SURGE_MANTISSA, "ItsARace: utilization too high");
    ...
    function liquidate(address borrower, uint amount) external {
        ...
        require(userCollateralRatioMantissa > _currentCollateralRatioMantissa, "ItsARace: borrower not liquidatable");
